 ...
 */
#define CLEAR_HASH(s) ippsSet_32s( -((Ipp32s)s->w_size), (Ipp32s*)s->head, (int)s->hash_size )
    ...
    uInt  dictLength;
{
    ...
    if( ippStsNoErr != ippsDeflateDictionarySet_8u( dictionary, length,
                              ...
                              mt_state* mt_s, deflate_state *s, IppLZ77Flush ipp_flush )
{
  ...
  do {
    if( src_offset > 0 ) { /* not first block */
      ippsSet_32s( -((Ipp32s)w_size), (Ipp32s*)head, (int)hash_size );
      if( src_offset >= w_size ) { /* the dictionary is in the source block fully */
        if( ippStsNoErr != ippsDeflateUpdateHash_8u( src - w_size, match_start - w_size, w_size + MIN_MATCH - 1,
        ...
        if( ippStsNoErr != ippsDeflateUpdateHash_8u( window,
    ...
    { /*block */
      ...
      if( ippStsNoErr != ippsDeflateLZ77_8u( (const Ipp8u**)&src, &mt_s->src_len, &match_start,
...
local void lz77_encode_call( uch** src, uInt* src_len, deflate_state *s, int flush )
{
  ...
  do {
    ...
    if( (*src_len >= MT_THRESHOLD * 2) && (dst_len >= MT_THRESHOLD) && (threads >= 2) ) {
      ...
      ippsZero_8u( (Ipp8u*)mt_s[1].lit_freq_tab, sizeof(IppDeflateFreqTable) * (L_CODES + D_CODES) * (chunks - 1) );
...
#pragma omp parallel for num_threads(chunks) reduction(|: ret)
      ...
      { /* block */
        ...
        for( i = 1; i < (int)chunks; i++, chunk_offset += chunk_step ) {
          ...
          ippsAdd_16u_ISfs( (const Ipp16u*)mt_s[i].lit_freq_tab, (Ipp16u*)s->dyn_ltree, L_CODES * 2, 0 );
          ippsAdd_16u_ISfs( (const Ipp16u*)mt_s[i].dist_freq_tab, (Ipp16u*)s->dyn_dtree, D_CODES * 2, 0 );
          ippsMove_8u( (const Ipp8u*)(lit_dst + chunk_offset),
                       (Ipp8u*)(lit_dst + dst_shift), (int)mt_s[i].dst_len );
          ippsMove_8u( (const Ipp8u*)(dist_dst + chunk_offset),
...
local void lz77_encode_call( uch** src, uInt* src_len, deflate_state *s, int flush )
{
  ...
  IppStatus status = ippsDeflateLZ77_8u( (const Ipp8u**)src, src_len, &s->match_start,
