...
#else
  jpeg_gen_optimal_table_intellib(cinfo, *htblptr, entropy->dc_count_ptrs[tbl]);
#endif
  did_dc[tbl] = TRUE;
      }
    }
...
#else
  jpeg_gen_optimal_table_intellib(cinfo, *htblptr, entropy->ac_count_ptrs[tbl]);
#endif
  did_ac[tbl] = TRUE;
      }
    }
...
#else
        jpeg_make_c_derived_tbl_intellib(cinfo, TRUE, tbl,
          & entropy->dc_derived_tbls[tbl]);
#endif
      }
...
#else
        jpeg_make_c_derived_tbl_intellib(cinfo, FALSE, tbl,
          & entropy->ac_derived_tbls[tbl]);
#endif
      }
    }
