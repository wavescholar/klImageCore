...
AperioFileManager::AperioFileManager(const char* fileName)
{
	tifFile=NULL;
	...
	sprintf(logFileName,"AperioReader.log");
	...
	catch(SVSTiffError e)
	{
		...
		err+="Caught SVSTiffError in AperioFileManager::(char* fileName) : ";
		err+=fileName;
		err+=e.what();
	...
	catch(...)
	{
		...
		ANSI_INFO_ss<<"ANSI COMPILE INFO: " <<__DATE__<<"     "<<__TIME__<<"   "<<__FILE__<<"   "<<__LINE__<<"       "<<std::endl;
		std::string err = ANSI_INFO_ss.str();
		err+="Caught non-SVSTiffError in AperioFileManager::AperioFileManager(char* fileName) : ";
		err+=fileName;
		logError( logFile, TEXT("\nAperioFileManager\n") ,err.c_str());flushall();throw err; 
	...
	if (!tifFile)
	{
		...
		ANSI_INFO_ss<<"ANSI COMPILE INFO: " <<__DATE__<<"     "<<__TIME__<<"   "<<__FILE__<<"   "<<__LINE__<<"       "<<std::endl;
		std::string err = ANSI_INFO_ss.str();
		err+="Bad Tiff File Handle n AperioFileManager::AperioFileManager(char* fileName)";
		err+=fileName;
		logError( logFile, TEXT("\nAperioFileManager\n") ,err.c_str());flushall();throw err; 
	}

	//logError( logFile, TEXT(" \n AperioFileManager ctor \n") ,"What?");

	delete logFileName;
}


void AperioFileManager::printTiffInfo(FILE* fname, TiffImageInfo info)
{
...
void AperioFileManager::writeTiffFile(char* filename,  void* raster, TiffImageInfo info ){
	TIFF *output;
	uint32 width, height;
	...
	else
	{
		...
		ANSI_INFO_ss<<"ANSI COMPILE INFO: " <<__DATE__<<"     "<<__TIME__<<"   "<<__FILE__<<"   "<<__LINE__<<"       "<<std::endl;
		std::string err = ANSI_INFO_ss.str();
		err+="AperioFileManager::writeTiffFile only supports 8 bit image data.";
		logError( logFile, TEXT("\nAperioFileManager\n") ,err.c_str());flushall();throw err; 
	}
...
TiffImageInfo AperioFileManager::getTiffImageInfo(unsigned int i)
{
	uint32 height=0;
	uint32 width=0;
	...
	try
	{
		...
		else
		{
			...
			ANSI_INFO_ss<<"ANSI COMPILE INFO: " <<__DATE__<<"     "<<__TIME__<<"   "<<__FILE__<<"   "<<__LINE__<<"       "<<std::endl;
			std::string err = ANSI_INFO_ss.str();
			err+="Problem with tif file handle in AperioFileManager::getTiffImageInfo";		
			logError( logFile, TEXT("\nAperioFileManager\n") ,err.c_str());flushall();throw err; 
	...
	catch(SVSTiffError e)
	{
		...
		ANSI_INFO_ss<<"ANSI COMPILE INFO: " <<__DATE__<<"     "<<__TIME__<<"   "<<__FILE__<<"   "<<__LINE__<<"       "<<std::endl;
		std::string err = ANSI_INFO_ss.str();
		err+="Caught SVSTiffError in AperioFileManager::getTiffImageInfo";
		err+=e.what();
		logError( logFile, TEXT("\nAperioFileManager\n") ,err.c_str());flushall();throw err; 
	}
	catch(...)
	{
		...
		ANSI_INFO_ss<<"ANSI COMPILE INFO: " <<__DATE__<<"     "<<__TIME__<<"   "<<__FILE__<<"   "<<__LINE__<<"       "<<std::endl;
		std::string err = ANSI_INFO_ss.str();
		err+="Problem in AperioFileManager::getTiffImageInfo";
		logError( logFile, TEXT("\nAperioFileManager\n") ,err.c_str());flushall();throw err; 
...
unsigned int AperioFileManager::getNumberOfTiffDirs()
{
	int numDirs=0;

	if(!tifFile)
	{
		throw "Bad tif file handle in AperioFileManager::getNumberOfTiffDirs";
	}
	else
	{
		try
		...
		catch(SVSTiffError e)
		{
			...
			ANSI_INFO_ss<<"ANSI COMPILE INFO: " <<__DATE__<<"     "<<__TIME__<<"   "<<__FILE__<<"   "<<__LINE__<<"       "<<std::endl;
			std::string err = ANSI_INFO_ss.str();
			err+="Caught SVSTiffError in AperioFileManager::getNumberOfTiffDirs";
			err+=e.what();
			logError( logFile, TEXT("\nAperioFileManager\n") ,err.c_str());flushall();throw err; 
		}
		catch(...)
		{
			...
			ANSI_INFO_ss<<"ANSI COMPILE INFO: " <<__DATE__<<"     "<<__TIME__<<"   "<<__FILE__<<"   "<<__LINE__<<"       "<<std::endl;
			std::string err = ANSI_INFO_ss.str();
			err+="Problem getting the number of tif dirs in  AperioFileManager::getNumberOfTiffDirs";
			logError( logFile, TEXT("\nAperioFileManager\n") ,err.c_str());flushall();throw err; 
		}
		return numDirs;
...
void AperioFileManager::setTiffDirectory(unsigned int i)
{
	try
	{
		if (!TIFFSetDirectory(tifFile, i))
		{
			...
			ANSI_INFO_ss<<"ANSI COMPILE INFO: " <<__DATE__<<"     "<<__TIME__<<"   "<<__FILE__<<"   "<<__LINE__<<"       "<<std::endl;
			std::string err = ANSI_INFO_ss.str();
			err+="Error setting tiff directory in AperioFileManager::setTiffDirectory";
			logError( logFile, TEXT("\nAperioFileManager\n") ,err.c_str());flushall();throw err; 
		}
	}
	catch( SVSTiffError e)
	{
		...
		ANSI_INFO_ss<<"ANSI COMPILE INFO: " <<__DATE__<<"     "<<__TIME__<<"   "<<__FILE__<<"   "<<__LINE__<<"       "<<std::endl;
		std::string err = ANSI_INFO_ss.str();
		err+="Caught SVSTiffError in AperioFileManager::setTiffDirectory(char* ofileName)";
		err+=e.what();
		logError( logFile, TEXT("\nAperioFileManager\n") ,err.c_str());flushall();throw err; 
	}
	catch(...)
...
bool AperioFileManager::getThumbnail(const char* ofileName)
{
	unsigned char* thumbnail = NULL;

	try
	{
		//The thumbnail is always the second tiff dir in an Aperio image.
		//It's guaranteed to be there accorind to the Aperio documentation.
	...
	catch(SVSTiffError e)
	{
		...
		ANSI_INFO_ss<<"ANSI COMPILE INFO: " <<__DATE__<<"     "<<__TIME__<<"   "<<__FILE__<<"   "<<__LINE__<<"       "<<std::endl;
		std::string err = ANSI_INFO_ss.str();
		err+="Caught SVSTiffError in AperioFileManager::getThumbnail(char* ofileName)";
		err+=e.what();
		logError( logFile, TEXT("\nAperioFileManager\n") ,err.c_str());flushall();throw err; 
	...
	catch(...)
	{
		...
		ANSI_INFO_ss<<"ANSI COMPILE INFO: " <<__DATE__<<"     "<<__TIME__<<"   "<<__FILE__<<"   "<<__LINE__<<"       "<<std::endl;
		std::string err = ANSI_INFO_ss.str();
		err+="Caught a non SVSTiffError in  AperioFileManager::getThumbnail(const char* ofileName)";
		logError( logFile, TEXT("\nAperioFileManager\n") ,err.c_str());flushall();throw err; 
	}
...
bool AperioFileManager::getLabelImage(const char* ofileName)
{
	...
	for(i=0;i<info.tiffDirs;i++)
	{
		...
		catch(SVSTiffError e)
		{
			...
			ANSI_INFO_ss<<"ANSI COMPILE INFO: " <<__DATE__<<"     "<<__TIME__<<"   "<<__FILE__<<"   "<<__LINE__<<"       "<<std::endl;
			std::string err = ANSI_INFO_ss.str();
			err+="Caught SVSTiffError in AperioFileManager::getLabelImage(char* ofileName)";
			err+=e.what();
			logError( logFile, TEXT("\nAperioFileManager\n") ,err.c_str());flushall();throw err; 
	...
	catch(...)
	{
		...
		ANSI_INFO_ss<<"ANSI COMPILE INFO: " <<__DATE__<<"     "<<__TIME__<<"   "<<__FILE__<<"   "<<__LINE__<<"       "<<std::endl;
		std::string err = ANSI_INFO_ss.str();
		err+="Caught non SVSTiffError in AperioFileManager::getLabelImage(char* ofileName)";
		logError( logFile, TEXT("\nAperioFileManager\n") ,err.c_str());flushall();throw err; 
	}
...
bool AperioFileManager::getWSIImage(const char* ofileName)
{
	...
	catch(...)
	{
		...
		ANSI_INFO_ss<<"ANSI COMPILE INFO: " <<__DATE__<<"     "<<__TIME__<<"   "<<__FILE__<<"   "<<__LINE__<<"       "<<std::endl;
		std::string err = ANSI_INFO_ss.str();
		err+="Problem in AperioFileManager::getWSIImage(char* ofileName)";
		logError( logFile, TEXT("\nAperioFileManager\n") ,err.c_str());flushall();throw err; 
	}
	return true;
}

std::map<std::string,std::string> AperioFileManager::getMetadata(unsigned int i)
{
	if(tifFile)
	{
		try
		...
		catch(...)
		{
			...
			ANSI_INFO_ss<<"ANSI COMPILE INFO: " <<__DATE__<<"     "<<__TIME__<<"   "<<__FILE__<<"   "<<__LINE__<<"       "<<std::endl;
			std::string err = ANSI_INFO_ss.str();
			err+="Problem Extracting Metadata in AperioFileManager::getMetadata";
			logError( logFile, TEXT("\nAperioFileManager\n") ,err.c_str());flushall();throw err; 
		}
	}
	else
	{
		...
		ANSI_INFO_ss<<"ANSI COMPILE INFO: " <<__DATE__<<"     "<<__TIME__<<"   "<<__FILE__<<"   "<<__LINE__<<"       "<<std::endl;
		std::string err = ANSI_INFO_ss.str();
		err+="Bad tifFile in AperioFileManager::getMetadata";
		logError( logFile, TEXT("\nAperioFileManager\n") ,err.c_str());flushall();throw err; 

	}
}

std::vector<std::string> AperioFileManager::parseImageDescriptionTag(std::string tag)
{
...
void AperioFileManager::ExtractMainResTile(const char * tiffoutputFile,const char * ppmoutputFile,unsigned int x,unsigned int y,unsigned int w,unsigned int h) 
{
	TiffImageInfo info =getTiffImageInfo(0);
	...
	if(x<0 || y <0 || w<=0 || h<=0) 
	{
		...
		ANSI_INFO_ss<<"ANSI COMPILE INFO: " <<__DATE__<<"     "<<__TIME__<<"   "<<__FILE__<<"   "<<__LINE__<<"       "<<std::endl;
		std::string err = ANSI_INFO_ss.str();
		err+="Negative image coords in void AperioFileManager::ExtractMainResTile.";
		logError( logFile, TEXT("\nAperioFileManager\n") ,err.c_str());
		flushall();
		throw err;
	...
	if(x+w > info.width || y+h > info.height) 
	{
		...
		ANSI_INFO_ss<<"ANSI COMPILE INFO: " <<__DATE__<<"     "<<__TIME__<<"   "<<__FILE__<<"   "<<__LINE__<<"       "<<std::endl;
		std::string err = ANSI_INFO_ss.str();
		err+="Region out of bounds in AperioFileManager::ExtractMainResTile";
		logError( logFile, TEXT("\nAperioFileManager\n") ,err.c_str());
		flushall();
		throw err;
	...
	if(bitdepth ==8)
	{
			...
			(...)
		{
			...
			ANSI_INFO_ss<<"ANSI COMPILE INFO: " <<__DATE__<<"     "<<__TIME__<<"   "<<__FILE__<<"   "<<__LINE__<<"       "<<std::endl;
			std::string err = ANSI_INFO_ss.str();
			err+="Error in AperioFileManager::ExtractMainResPPM(const char * outputFile) - memory allocation error.";
			logError( logFile, TEXT("\nAperioFileManager\n") ,err.c_str());flushall();throw err; 
		}
		int tilebuf_start_y ; 
		int tilebuf_end_y ; 
...
void AperioFileManager::ExtractMainResTileApplyICC(const char * outputFile,unsigned int x,unsigned int y,unsigned int w,unsigned int h) 
{
	unsigned char * databuf =0;
	unsigned char * tilebuf =0;
	...
	try
	{
			...
			(...)//bbcrevisit - instrument SVS and klIPP with klnew handler.
		{
			...
			ANSI_INFO_ss<<"ANSI COMPILE INFO: " <<__DATE__<<"     "<<__TIME__<<"   "<<__FILE__<<"   "<<__LINE__<<"       "<<std::endl;
			std::string err = ANSI_INFO_ss.str();
			err+="Error in AperioFileManager::ExtractMainResTileApplyICC(const char * outputFile) - memory allocation error.";
			logError( logFile, TEXT("\nAperioFileManager\n") ,err.c_str());flushall();throw err; 
		}
		...
		ANSI_INFO_ss<<"ANSI COMPILE INFO: " <<__DATE__<<"     "<<__TIME__<<"   "<<__FILE__<<"   "<<__LINE__<<"       "<<std::endl;
		std::string err = ANSI_INFO_ss.str();
		err+="No ICC profile found in AperioFileManager::ExtractMainResTileApplyICC";
		logError( logFile, TEXT("\nAperioFileManager\n") ,err.c_str());flushall();throw err; 
		...
		{
			err+= "Problem with AddXform for source Possible Invalid Profile in AperioFileManager::ExtractMainResTileApplyICC";
			logError( logFile, TEXT("\nAperioFileManager\n") ,err.c_str());flushall();throw err; 
		}
		if (szDstProfile && *szDstProfile && cmm.AddXform(szDstProfile)) 
		{
			err+="Problem with AddXform for destination profile Possible Invalid Profile in AperioFileManager::ExtractMainResTileApplyICC";
			logError( logFile, TEXT("\nAperioFileManager\n") ,err.c_str());flushall();throw err; 
		}
		if (cmm.Begin() != icCmmStatOk) 
		{
			err+="Bad Icc status in AperioFileManager::ExtractMainResPPM_ApplyICC";
			logError( logFile, TEXT("\nAperioFileManager\n") ,err.c_str());flushall();throw err; 
		}
		sn =3;
		...
		default:
			{
				ANSI_INFO_ss<<"ANSI COMPILE INFO: " <<__DATE__<<"     "<<__TIME__<<"   "<<__FILE__<<"   "<<__LINE__<<"       "<<std::endl;
				std::string err = ANSI_INFO_ss.str();
				err+="Bad Destination Colorspace in AperioFileManager::ExtractMainResTileApplyICC";
				logError( logFile, TEXT("\nAperioFileManager\n") ,err.c_str());flushall();throw err; 
			}
		}
		//////////////////END ICC ///////////////////////////////////
	...
	catch(...)
	{
		std::stringstream ANSI_INFO_ss (std::stringstream::in | std::stringstream::out );
		ANSI_INFO_ss<<"ANSI COMPILE INFO: " <<__DATE__<<"     "<<__TIME__<<"   "<<__FILE__<<"   "<<__LINE__<<"       "<<std::endl;
		std::string err = ANSI_INFO_ss.str()+ "Caught non- SVSTiffError in AperioFileManager::ExtractMainResTileApplyICC";
		logError( logFile, TEXT("\nAperioFileManager\n") ,err.c_str());flushall();throw err; 
	}
...
bool AperioFileManager::getTiffDirImage(unsigned int dir, const char* tiffoutputFile)
{
...
bool AperioFileManager::getStripSubDir(unsigned int subDir,const char* ofileName)
{
	unsigned char* thumbnail = NULL;
...
void AperioFileManager::printMetadata()
{
	TiffImageInfo info = getTiffImageInfo(0);
